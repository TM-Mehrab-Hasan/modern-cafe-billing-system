...
Class Action {
	...
	function get_user(){
		extract($_POST);
		$user = $this->db->query("SELECT * FROM users WHERE id = '$id'")->fetch_array();
		if($user){
	...
	function save_category(){
		extract($_POST);
		// Server-side permission check: only admin (type 1) can save categories
	...
	function delete_category(){
		extract($_POST);
		// Server-side permission check: only admin (type 1) can delete categories
		if(!isset($_SESSION['login_type']) || $_SESSION['login_type'] != 1){
			// unauthorized
			echo json_encode(array('status'=>0,'msg'=>'Unauthorized'));
			return;
	...
	function save_product(){
		extract($_POST);
		// Server-side permission check: only admin (type 1) can save products
		if(!isset($_SESSION['login_type']) || $_SESSION['login_type'] != 1){
			// unauthorized
			echo json_encode(array('status'=>0,'msg'=>'Unauthorized'));
			return;
	...
	function delete_product(){
		extract($_POST);
		// Server-side permission check: only admin (type 1) can delete products
		if(!isset($_SESSION['login_type']) || $_SESSION['login_type'] != 1){
			// unauthorized
			echo json_encode(array('status'=>0,'msg'=>'Unauthorized'));
			return;
	...
	function delete_order(){
		extract($_POST);
		// Server-side permission check: only admin (type 1) can delete orders
		if(!isset($_SESSION['login_type']) || $_SESSION['login_type'] != 1){
			echo json_encode(array('status'=>0,'msg'=>'Unauthorized'));
			return;
		}
